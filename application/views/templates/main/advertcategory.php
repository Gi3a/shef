	...
	<div class="block block-center block-row block-100 margin-1">
		<div class="block block-20 block-center">
			...
			<select name="category" onchange="top.location=this.value">
				<option <?php if(!empty($this->route['category']) and $this->route['category'] == '') {echo 'selected';} ?> value="/adverts">Все</option>
				<option <?php if(!empty($this->route['category']) and $this->route['category'] == 'soup') {echo 'selected';} ?> value="/adverts/soup">Супы</option>
				<option <?php if(!empty($this->route['category']) and $this->route['category'] == 'breakfast') {echo 'selected';} ?> value="/adverts/breakfast">Завтраки</option>
				<option <?php if(!empty($this->route['category']) and $this->route['category'] == 'snack') {echo 'selected';} ?> value="/adverts/snack">Закуски</option>
			</select>
		</div>
		<div class="block block-20 block-center">
			<span>Сортировать</span>
			<form action="/adverts/<?php echo $this->route['category']; ?>/" method="GET">
